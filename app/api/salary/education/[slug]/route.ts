...
export async function GET(
  ...
  try {
    const buildQuery = (where: string) => `
      ...
      WHERE ${where}
      ORDER BY d.survey_year DESC, aw.sex, aw.enterprise_size, aw.age_group`

    // education固定・age_group='学歴計'のデータを取得
    let rows: any[] = await query(
      buildQuery("aw.education = ? AND aw.age_group = '学歴計'"),
      [slug]
    ) as any[]

    if (rows.length === 0) {
      return NextResponse.json({ success: false, message: '学歴が見つかりません' }, { status: 404 })
    }
