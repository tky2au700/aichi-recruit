...
// 男性・女性ランキング共用API
// sex クエリパラメーターで '男' / '女' を受け取る
export async function GET(req: NextRequest) {
  const { searchParams } = new URL(req.url)
  const sex            = searchParams.get('sex')            || '男'
  ...
  try {
    ...
    const [rows, statsRows] = await Promise.all([
      query(
        ...
        age_group: string; sex: string; enterprise_size: string; education: string
      ...
      query(
        `SELECT AVG(annual_income) as avg_income, MAX(annual_income) as max_income,
         ...
         WHERE dataset_id = ? AND sex = ? AND enterprise_size = ?
           AND education = ? AND age_group != '学歴計'`,
        [targetDatasetId, sex, enterpriseSize, education]
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        sex, enterprise_size: enterpriseSize, education,
