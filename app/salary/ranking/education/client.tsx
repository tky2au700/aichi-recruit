...
export function EducationRankingClient({ initialSex, initialSize, initialYear, initialSort, initialDir, pageHeading, pageDescription }: Props = {}) {
  const router   = useRouter()
  const pathname = usePathname()

  const [data, setData]       = useState<EducationRow[]>([])
  ...
  const fetchData = useCallback(async (_sex: string, _size: string, _year: number | null) => {
    ...
    try {
      ...
      const res  = await fetch(`/api/salary/ranking/education?${params}`)
  ...
  function handleSort(key: SortKey) {
    if (sortKey === key) {
      ...
      setSortKey(key); setSortDir('desc'); pushUrl(sex, size, surveyYear, key, 'desc')
    }
  }

  const sortedData = [...data].sort((a, b) => {
    const av = a[sortKey as keyof EducationRow] as number | null
    const bv = b[sortKey as keyof EducationRow] as number | null
  ...
  const S = {
    ...
    chipActive: { padding: '5px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: '1.5px solid #16a34a', background: '#F0FDF4', color: '#16a34a', transition: 'all .15s' },
  ...
  function Th({ label, k }: { label: string; k: SortKey }) {
    ...
    return (
      <th style={{ ...S.th, color: isActive ? '#16a34a' : '#64748B' }} onClick={() => handleSort(k)}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {label}
          {isActive
            ? sortDir === 'desc' ? <ChevronDown size={13} style={{ color: '#16a34a' }} /> : <ChevronUp size={13} style={{ color: '#16a34a' }} />
  ...
  const baseTitle = `学歴別平均${currentSortLabel}ランキング${currentYearStr}`
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        {meta && !loading && (
          <div style={S.kpiGrid}>
            {[
              { icon: <Award size={15} color="#16a34a" />, label: '最高年収', value: fmtWan(meta.max_income), sub: 'トップ学歴区分' },
              ...
              { icon: <Users size={15} color="#0891b2" />, label: '労働者数', value: meta.total_workers ? `${(Number(meta.total_workers) / 10000).toFixed(0)}万人` : '−', sub: '対象労働者の合計' },
          ...
          <div style={S.filterGroup}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {SEX_OPTIONS.map(o => (
                <button key={o.value}
                  style={sex === o.value
                    ? { ...S.chipActive, border: o.value === '男' ? '1.5px solid #1a73e8' : o.value === '女' ? '1.5px solid #DB4437' : '1.5px solid #16a34a', color: o.value === '男' ? '#1a73e8' : o.value === '女' ? '#DB4437' : '#16a34a', background: o.value === '男' ? '#EBF3FE' : o.value === '女' ? '#FCECEA' : '#F0FDF4' }
                    : S.chip}
        ...
        <div style={S.tableCard}>
          <div style={S.tableHead}>
            <div style={S.tableTitle}>
              <span>学歴別{currentSortLabel}ランキング</span>
              ...
              <table style={S.table}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...S.th, minWidth: 140, cursor: 'default' }}>学歴</th>
                ...
                <tbody>
                  {sortedData.map((row, idx) => {
                    const sortVal   = row[sortKey as keyof EducationRow] as number | null
                    ...
                    return (
                      <tr key={`${row.education}-${idx}`}
                        ...
                        <td style={{ ...S.td, minWidth: 140 }}>
                          ...
                          {sortKey === 'annual_income' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : '#16a34a', borderRadius: 4 }} /></div>}
                        </td>
                        {/* 月給 */}
                        <td style={{ ...S.td, minWidth: 120 }}>
                          <span style={{ fontWeight: sortKey === 'monthly_wage' ? 700 : 400, color: sortKey === 'monthly_wage' ? (idx === 0 ? '#D97706' : '#16a34a') : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {fmtWan(row.monthly_wage)}
                          </span>
                          {sortKey === 'monthly_wage' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : '#16a34a', borderRadius: 4 }} /></div>}
                        </td>
                        {/* 年間賞与 */}
                        <td style={{ ...S.td, minWidth: 110 }}>
                          <span style={{ fontWeight: sortKey === 'annual_bonus' ? 700 : 400, color: sortKey === 'annual_bonus' ? (idx === 0 ? '#D97706' : '#16a34a') : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {fmtWan(row.annual_bonus)}
                          </span>
                          {sortKey === 'annual_bonus' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : '#16a34a', borderRadius: 4 }} /></div>}
                        </td>
                        {/* 平均年齢 */}
                        <td style={S.td}>
                          <span style={{ fontWeight: sortKey === 'age' ? 700 : 400, color: sortKey === 'age' ? (idx === 0 ? '#D97706' : '#16a34a') : '#475569' }}>
                            {fmtNum(row.age, '歳')}
                          </span>
                        ...
                        {/* 勤続年数 */}
                        <td style={S.td}>
                          <span style={{ fontWeight: sortKey === 'tenure_years' ? 700 : 400, color: sortKey === 'tenure_years' ? (idx === 0 ? '#D97706' : '#16a34a') : '#475569' }}>
                        ...
                        <td style={S.td}>
                          ...
                          {sortKey === 'hourly_wage' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : '#16a34a', borderRadius: 4 }} /></div>}
                        </td>
                      </tr>
                    )
                  })}
