...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `男性の職種別平均年収ランキング${yearStr} | AIリクルート`,
    description: `${yearStr}調査の賃金構造基本統計調査に基づく男性の職種別平均年収ランキング。各職種の男性労働者の年収・月給・賞与・勤続年数を比較できます。`,
    keywords: ['男性 職種別年収', '男性 職種 年収ランキング', '男性労働者 職種別', '男性 職業別年収'],
    path: '/salary/ranking/occupation/male',
  })
}

export default async function OccupationMaleRankingPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const pageHeading = sizeName
    ? `${sizeName}の男性職種別平均${sortLabel}ランキング${yearStr}`
    : `男性の職種別平均${sortLabel}ランキング${yearStr}`
