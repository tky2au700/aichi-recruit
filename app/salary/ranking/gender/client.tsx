...
export function GenderRankingClient({ fixedSex, initialSize, initialYear, initialSort, initialDir, pageHeading }: Props) {
  ...
  // アクセントカラー: 男=青, 女=赤
  const accent = fixedSex === '男' ? '#1a73e8' : '#DB4437'
  const accentBg = fixedSex === '男' ? '#EBF3FE' : '#FCECEA'

  const pushUrl = useCallback((
    newSize: string, newYear: number | null,
  ...
  const fetchData = useCallback(async (_size: string, _year: number | null) => {
    setLoading(true); setError(null)
    try {
      const params = new URLSearchParams({ sex: fixedSex, enterprise_size: _size })
      if (_year) params.set('survey_year', String(_year))
      const res  = await fetch(`/api/salary/ranking/gender?${params}`)
  ...
  useEffect(() => { fetchData(size, surveyYear) }, [size, surveyYear, fetchData])

  function handleSort(key: SortKey) {
    if (sortKey === key) {
      const newDir: SortDir = sortDir === 'desc' ? 'asc' : 'desc'
      setSortDir(newDir); pushUrl(size, surveyYear, key, newDir)
    } else {
      setSortKey(key); setSortDir('desc'); pushUrl(size, surveyYear, key, 'desc')
    }
  }

  const sortedData = [...data].sort((a, b) => {
    const av = a[sortKey as keyof GenderRow] as number | null
    const bv = b[sortKey as keyof GenderRow] as number | null
    if (av == null && bv == null) return 0
    if (av == null) return 1; if (bv == null) return -1
    return sortDir === 'desc' ? bv - av : av - bv
  })

  const topSortValue = sortedData[0]?.[sortKey] as number | null ?? null
  ...
  const S = {
    ...
    chipActive: { padding: '5px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1.5px solid ${accent}`, background: accentBg, color: accent, transition: 'all .15s' },
  ...
  function Th({ label, k }: { label: string; k: SortKey }) {
    ...
    return (
      <th style={{ ...S.th, color: isActive ? accent : '#64748B' }} onClick={() => handleSort(k)}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {label}
          {isActive
            ? sortDir === 'desc' ? <ChevronDown size={13} style={{ color: accent }} /> : <ChevronUp size={13} style={{ color: accent }} />
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        {meta && !loading && (
          <div style={S.kpiGrid}>
            {[
              { icon: <Award size={15} color={accent} />, label: '最高年収', value: fmtWan(meta.max_income), sub: `トップ年齢階級（${sexLabel}）` },
              { icon: <BarChart2 size={15} color="#0F9D58" />, label: `${sexLabel}平均年収`, value: fmtWan(meta.avg_income), sub: `${meta.row_count}区分の平均` },
              { icon: <TrendingUp size={15} color="#F4B400" />, label: '集計区分数', value: `${meta.row_count}区分`, sub: `${meta.survey_year}年調査` },
              { icon: <Users size={15} color={accent} />, label: `${sexLabel}労働者数`, value: meta.total_workers ? `${(Number(meta.total_workers) / 10000).toFixed(0)}万人` : '−', sub: '対象労働者の合計' },
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {years.map(y => (
                <button key={y.survey_year} style={surveyYear === y.survey_year ? S.chipActive : S.chip}
                  onClick={() => { setSurveyYear(y.survey_year); pushUrl(size, y.survey_year, sortKey, sortDir) }}>
                  {y.survey_year}年
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {SIZE_OPTIONS.map(o => (
                <button key={o.value} style={size === o.value ? S.chipActive : S.chip}
                  onClick={() => { setSize(o.value); pushUrl(o.value, surveyYear, sortKey, sortDir) }}>
                  {o.label}
                </button>
              ))}
            </div>
        ...
        <div style={S.tableCard}>
          <div style={S.tableHead}>
            <div style={S.tableTitle}>
              <span style={{ color: accent }}>{sexLabel}</span>
              <span>{currentSortLabel}ランキング（年齢階級別）</span>
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {sortedData.map((row, idx) => {
                    const sortVal   = row[sortKey as keyof GenderRow] as number | null
                    ...
                    return (
                      <tr key={`${row.age_group}-${idx}`}
                        ...
                        <td style={{ ...S.td, minWidth: 140 }}>
                          <span style={{ fontWeight: 700, fontSize: 13, color: sortKey === 'annual_income' ? (idx === 0 ? '#D97706' : isAboveAvg ? accent : '#374151') : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {fmtWan(row.annual_income)}
                          </span>
                          {sortKey === 'annual_income' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : accent, borderRadius: 4 }} /></div>}
                        </td>
                        <td style={{ ...S.td, minWidth: 120 }}>
                          <span style={{ fontWeight: sortKey === 'monthly_wage' ? 700 : 400, color: sortKey === 'monthly_wage' ? (idx === 0 ? '#D97706' : accent) : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {fmtWan(row.monthly_wage)}
                          </span>
                          {sortKey === 'monthly_wage' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : accent, borderRadius: 4 }} /></div>}
                        </td>
                        <td style={{ ...S.td, minWidth: 110 }}>
                          <span style={{ fontWeight: sortKey === 'annual_bonus' ? 700 : 400, color: sortKey === 'annual_bonus' ? (idx === 0 ? '#D97706' : accent) : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {fmtWan(row.annual_bonus)}
                          </span>
                          {sortKey === 'annual_bonus' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : accent, borderRadius: 4 }} /></div>}
                        </td>
                        <td style={S.td}>
                          <span style={{ fontWeight: sortKey === 'age' ? 700 : 400, color: sortKey === 'age' ? (idx === 0 ? '#D97706' : accent) : '#475569' }}>{fmtNum(row.age, '歳')}</span>
                          {sortKey === 'age' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : '#94A3B8', borderRadius: 4 }} /></div>}
                        </td>
                        <td style={S.td}>
                          <span style={{ fontWeight: sortKey === 'tenure_years' ? 700 : 400, color: sortKey === 'tenure_years' ? (idx === 0 ? '#D97706' : accent) : '#475569' }}>{fmtNum(row.tenure_years, '年')}</span>
                        ...
                        <td style={S.td}>
                          <span style={{ fontWeight: sortKey === 'hourly_wage' ? 700 : 400, color: sortKey === 'hourly_wage' ? (idx === 0 ? '#D97706' : accent) : '#475569', fontVariantNumeric: 'tabular-nums' }}>
                            {row.hourly_wage != null ? `${row.hourly_wage.toLocaleString()}円` : '−'}
                          </span>
                          {sortKey === 'hourly_wage' && <div style={S.barWrap}><div style={{ width: `${sortRatio}%`, height: '100%', background: idx === 0 ? '#F4B400' : accent, borderRadius: 4 }} /></div>}
                        </td>
                      </tr>
                    )
                  })}
