...
import {
  ...
  BarChart2, ArrowLeft, TrendingDown, Building2, Info,
} from 'lucide-react'
import {
...
const ENTERPRISE_ORDER = ['企業規模計', '1,000人以上', '100～999人', '10～99人']
const SEX_ORDER        = ['計', '男', '女']
const SEX_LABEL: Record<string, string> = { '計': '男女計', '男': '男性', '女': '女性' }
const SEX_COLOR: Record<string, string> = { '計': '#7c3aed', '男': '#0ea5e9', '女': '#e8336d' }
const ACCENT = '#7c3aed'
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', borderLeft: `3px solid ${ACCENT}`, paddingLeft: 10, margin: 0 }}>
        {children}
      </h2>
    </div>
  )
...
const METRIC_TABS = [
  ...
  { key: 'scheduled_wage', label: '月給',      unit: '万円', divisor: 10 },
  { key: 'annual_bonus',   label: '年間賞与',  unit: '万円', divisor: 10 },
  { key: 'overtime_hours', label: '残業時間',  unit: 'h',    divisor: 1  },
  { key: 'tenure_years',   label: '勤続年数',  unit: '年',   divisor: 1  },
] as const
...
const LINE_COLORS = [ACCENT, '#db2777', '#1a73e8', '#d97706', '#16a34a', '#64748b']
const SEX_LINES   = ['計', '男', '女']
const SEX_LABELS: Record<string, string> = { '計': '男女計', '男': '男性', '女': '女性' }
const SIZE_LINES  = ['企業規模計', '1,000人以上', '100～999人', '10～99人']
...
function TrendChart({ timeSeriesAll, growthStr, growthPositive, oldest, latest }: {
  ...
  const formatTick = (v: number) => {
    ...
    if (metricDef.unit === '年')  return `${v}年`
    return `${v}`
  }
  const formatTooltip = (v: number, name: string) => {
    ...
    if (metricDef.unit === '年')  return [`${v}年`, name]
    return [`${v}`, name]
  }
  ...
  return (
    <section style={{ marginBottom: 36 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', borderLeft: `3px solid ${ACCENT}`, paddingLeft: 10, margin: 0 }}>{chartTitle}</h2>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {METRIC_TABS.map(m => (
          <button key={m.key} onClick={() => setMetric(m.key)} style={{
            ...
            border: metric === m.key ? `1.5px solid ${ACCENT}` : '1.5px solid #E2E8F0',
            background: metric === m.key ? '#ede9fe' : '#fff',
            color: metric === m.key ? ACCENT : '#475569',
            transition: 'all 0.15s',
          }}>{m.label}</button>
        ))}
      </div>
...
// ---------- メインコンポーネント ----------
export function AgeGroupDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/salary/age-group/${encodeURIComponent(slug)}`)
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
        <div style={{ width: 36, height: 36, border: '3px solid #E2E8F0', borderTopColor: ACCENT, borderRadius: '50%', margin: '0 auto 16px', animation: 'spin 0.8s linear infinite' }} />
  ...
  if (error || !data) {
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <Link href="/salary/ranking/age-group" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: ACCENT, color: '#fff', padding: '10px 20px', borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
          <ArrowLeft size={15} />年齢別ランキングに戻る
        </Link>
      </div>
    )
  }
  ...
  return (
    <main style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      {/* ヒーローバナー */}
      <div style={{ background: 'linear-gradient(135deg, #f5f3ff 0%, #ffffff 70%)', borderBottom: '1px solid #E2E8F0' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px' }}>
          <nav aria-label="パンくずリスト" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '20px 0 0', fontSize: 12, color: '#64748B', flexWrap: 'wrap' }}>
            <Link href="/" style={{ color: ACCENT, textDecoration: 'none' }}>AIリクルート</Link>
            <ChevronRight size={12} />
            <Link href="/salary/ranking/age-group" style={{ color: ACCENT, textDecoration: 'none' }}>年齢別年収ランキング</Link>
            <ChevronRight size={12} />
            <span style={{ color: '#94A3B8' }}>{data.age_group}</span>
          </nav>
          <div style={{ padding: '24px 0 32px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 600, background: '#ede9fe', color: ACCENT, padding: '3px 10px', borderRadius: 20, border: '1px solid #ddd6fe' }}>
            ...
            <h1 style={{ fontSize: 30, fontWeight: 800, color: '#0F172A', margin: '0 0 8px', letterSpacing: '-0.5px', lineHeight: 1.25 }}>
              {data.age_group}の平均年収
            </h1>
            <p style={{ fontSize: 13, color: '#64748B', margin: 0 }}>
              {data.survey_group_name} / {data.latest_year}年調査
              {repFixed?.annual_income != null && (
                <> — 推定年収 <strong style={{ color: ACCENT, fontSize: 15 }}>{fmtWan(repFixed.annual_income)}</strong>（男女計・企業規模計）</>
              )}
            </p>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <section style={{ marginBottom: 36 }}>
            ...
            <div style={{ display: 'flex', background: '#F1F5F9', borderRadius: 24, padding: 3, gap: 2 }}>
              {SEX_ORDER.map(sex => (
                <button key={sex} onClick={() => setKpiSexTab(sex)} style={{
                  ...
                  cursor: 'pointer', boxShadow: kpiSexTab === sex ? '0 1px 4px rgba(0,0,0,0.10)' : 'none', transition: 'all 0.15s',
            ...
            <div style={{ display: 'flex', background: '#F1F5F9', borderRadius: 24, padding: 3, gap: 2, flexWrap: 'wrap' }}>
              {ENTERPRISE_ORDER.map(size => (
                <button key={size} onClick={() => setKpiSizeTab(size)} style={{
                  ...
                  color: kpiSizeTab === size ? ACCENT : '#64748B',
                  cursor: 'pointer', boxShadow: kpiSizeTab === size ? '0 1px 4px rgba(0,0,0,0.10)' : 'none', transition: 'all 0.15s',
                }}>{size}</button>
              ))}
            </div>
          </div>
          {rep && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
              <KpiCard icon={<Award size={13} color={ACCENT} />} label="推定年収" value={fmtWan(rep.annual_income)} sub="学歴計・企業規模計" accent={ACCENT} />
              ...
              <KpiCard icon={<Clock size={13} color="#0ea5e9" />} label="時給換算" value={(() => {
                const mw = rep.monthly_wage; if (mw == null) return '−'
                return `${Math.round(mw * 1000 / 160).toLocaleString()}円`
              })()} sub="月給 ÷ 160時間" />
              <KpiCard icon={<Users size={13} color={ACCENT} />} label="平均年齢" value={fmtFixed(rep.age, 1, '歳')} sub={`勤続 ${fmtFixed(rep.tenure_years, 1, '年')}`} />
              <KpiCard icon={<Clock size={13} color={rep.overtime_hours != null && rep.overtime_hours > 20 ? '#dc2626' : '#94A3B8'} />}
                label="月残業時間" value={fmtFixed(rep.overtime_hours, 1, 'h')}
              ...
              <KpiCard icon={<Clock size={13} color="#0f766e" />} label="月実労働時間" value={fmtFixed(rep.scheduled_hours, 1, 'h')} sub="所定内労働時間" />
              <KpiCard icon={<Users size={13} color="#64748B" />} label="労働者数" value={(() => {
                if (rep.workers == null) return '−'
                const p = rep.workers * 10
                return p >= 10000 ? `${(p / 10000).toFixed(1)}万人` : `${p.toLocaleString()}人`
              })()} sub="調査対象労働者数" />
            </div>
          )}
        </section>
        ...
        {(data.time_series_all ?? data.time_series).length > 1 && (
          <TrendChart timeSeriesAll={data.time_series_all ?? data.time_series} growthStr={growthStr} growthPositive={growthPositive} oldest={oldest} latest={latest} />
        )}
                ...
                <thead>
                  <tr style={{ background: '#F8FAFC' }}>
                    {['企業規模', '推定年収', '月給（所定内）', '年間賞与', '時給換算', '平均年齢', '勤続年数', '月残業', '労働者数'].map(h => (
                      <th key={h} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#64748B', borderBottom: '1px solid #E2E8F0', textAlign: 'left', whiteSpace: 'nowrap', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {sizeRows.map((r, i) => {
                    ...
                    const hourly = r.monthly_wage != null ? Math.round(Number(r.monthly_wage) * 1000 / 160) : null
                    return (
                      <tr key={r.enterprise_size} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFC', borderBottom: i < sizeRows.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                        <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: isTotal ? 700 : 500, color: '#0F172A', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <Building2 size={13} color={isTotal ? ACCENT : '#94A3B8'} />{r.enterprise_size}
                        </td>
                        ...
                        <td style={{ padding: '11px 14px', fontSize: 13, color: '#374151', whiteSpace: 'nowrap' }}>{hourly != null ? `${hourly.toLocaleString()}円/h` : '−'}</td>
                        ...
                        <td style={{ padding: '11px 14px', fontSize: 13, whiteSpace: 'nowrap', color: r.overtime_hours != null && r.overtime_hours > 20 ? '#dc2626' : '#374151', fontWeight: r.overtime_hours != null && r.overtime_hours > 20 ? 700 : 400 }}>{fmtFixed(r.overtime_hours, 1, 'h')}</td>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {ENTERPRISE_ORDER.map(size => (
              <button key={size} onClick={() => setSizeTab(size)} style={{
                ...
                border: sizeTab === size ? `1.5px solid ${ACCENT}` : '1.5px solid #E2E8F0',
                background: sizeTab === size ? '#ede9fe' : '#fff',
                color: sizeTab === size ? ACCENT : '#475569',
                cursor: 'pointer', transition: 'all 0.15s',
              }}>{size}</button>
            ))}
          </div>
                  ...
                  {sexRows.map((r, i) => {
                    ...
                    return (
                      <tr key={r.sex} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFC', borderBottom: i < sexRows.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                        <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: isTotal ? 700 : 500, color: sexColor, whiteSpace: 'nowrap' }}>{SEX_LABEL[r.sex] ?? r.sex}</td>
                        <td style={{ padding: '11px 14px', fontSize: 14, fontWeight: 700, color: sexColor, whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{fmtWan(r.annual_income)}</td>
                        <td style={{ padding: '11px 14px', fontSize: 13, color: '#374151', whiteSpace: 'nowrap' }}>{fmtWan(r.scheduled_wage)}</td>
                        <td style={{ padding: '11px 14px', fontSize: 13, color: '#374151', whiteSpace: 'nowrap' }}>{fmtWan(r.annual_bonus)}</td>
                        <td style={{ padding: '11px 14px', fontSize: 13, color: '#374151', whiteSpace: 'nowrap' }}>{hourly != null ? `${hourly.toLocaleString()}円/h` : '−'}</td>
                        <td style={{ padding: '11px 14px', fontSize: 13, color: '#374151', whiteSpace: 'nowrap' }}>{fmtFixed(r.age, 1, '歳')}</td>
        ...
        <section style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8 }}>
            {[
              { href: '/salary/ranking/age-group', label: '年齢別年収ランキング',   icon: <TrendingUp size={14} color={ACCENT} /> },
              { href: '/salary/ranking/education', label: '学歴別年収ランキング',   icon: <TrendingUp size={14} color="#16a34a" /> },
              { href: '/salary/ranking/male',      label: '男性年収ランキング',     icon: <TrendingUp size={14} color="#1a73e8" /> },
              { href: '/salary/ranking/female',    label: '女性年収ランキング',     icon: <TrendingUp size={14} color="#e8336d" /> },
              { href: '/salary/ranking/occupation', label: '職種別年収ランキング',  icon: <TrendingUp size={14} color="#0f766e" /> },
            ].map(({ href, label, icon }) => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 8 }}>
          <Link href="/salary/ranking/age-group" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: ACCENT, fontSize: 14, textDecoration: 'none', fontWeight: 500 }}>
            <ArrowLeft size={15} />年齢別ランキング一覧に戻る
          </Link>
        </div>
