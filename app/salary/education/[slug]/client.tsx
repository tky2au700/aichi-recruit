...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <h2 style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', borderLeft: '3px solid #16a34a', paddingLeft: 10, margin: 0 }}>
        {children}
      </h2>
    </div>
  )
...
const LINE_COLORS = ['#16a34a', '#db2777', '#1a73e8', '#d97706', '#7c3aed', '#64748b']
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {METRIC_TABS.map(m => (
          <button key={m.key} onClick={() => setMetric(m.key)} style={{
            ...
            border: metric === m.key ? '1.5px solid #16a34a' : '1.5px solid #E2E8F0',
            background: metric === m.key ? '#dcfce7' : '#fff',
            color: metric === m.key ? '#16a34a' : '#475569',
            transition: 'all 0.15s',
          }}>{m.label}</button>
        ))}
      </div>
...
// ---------- メインコンポーネント ----------
export function EducationDetailClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/salary/education/${encodeURIComponent(slug)}`)
      .then(r => r.json())
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
        <div style={{ width: 36, height: 36, border: '3px solid #E2E8F0', borderTopColor: '#16a34a', borderRadius: '50%', margin: '0 auto 16px', animation: 'spin 0.8s linear infinite' }} />
  ...
  if (error || !data) {
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
        ...
        <Link href="/salary/ranking/education" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#16a34a', color: '#fff', padding: '10px 20px', borderRadius: 8, fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>
          <ArrowLeft size={15} />学歴ランキングに戻る
        </Link>
      </div>
    )
  }
  ...
  return (
    <main style={{ background: '#F8FAFC', minHeight: '100vh' }}>
      {/* ヒーローバナー */}
      <div style={{ background: 'linear-gradient(135deg, #f0fdf4 0%, #ffffff 70%)', borderBottom: '1px solid #E2E8F0' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px' }}>
          <nav aria-label="パンくずリスト" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '20px 0 0', fontSize: 12, color: '#64748B', flexWrap: 'wrap' }}>
            <Link href="/" style={{ color: '#16a34a', textDecoration: 'none' }}>AIリクルート</Link>
            <ChevronRight size={12} />
            <Link href="/salary/ranking/education" style={{ color: '#16a34a', textDecoration: 'none' }}>学歴別年収ランキング</Link>
            <ChevronRight size={12} />
            <span style={{ color: '#94A3B8' }}>{data.education}</span>
          </nav>
          <div style={{ padding: '24px 0 32px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 600, background: '#dcfce7', color: '#16a34a', padding: '3px 10px', borderRadius: 20, border: '1px solid #bbf7d0' }}>
            ...
            <h1 style={{ fontSize: 30, fontWeight: 800, color: '#0F172A', margin: '0 0 8px', letterSpacing: '-0.5px', lineHeight: 1.25 }}>
              {data.education}の平均年収
            </h1>
            <p style={{ fontSize: 13, color: '#64748B', margin: 0 }}>
              {data.survey_group_name} / {data.latest_year}年調査
              {repFixed?.annual_income != null && (
                <> — 推定年収 <strong style={{ color: '#16a34a', fontSize: 15 }}>{fmtWan(repFixed.annual_income)}</strong>（男女計・企業規模計）</>
              )}
            </p>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <section style={{ marginBottom: 36 }}>
            ...
            <div style={{ display: 'flex', background: '#F1F5F9', borderRadius: 24, padding: 3, gap: 2, flexWrap: 'wrap' }}>
              {ENTERPRISE_ORDER.map(size => (
                <button key={size} onClick={() => setKpiSizeTab(size)} style={{
                  ...
                  color: kpiSizeTab === size ? '#16a34a' : '#64748B',
          ...
          {rep && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
              <KpiCard icon={<Award size={13} color="#16a34a" />} label="推定年収" value={fmtWan(rep.annual_income)} sub="学歴計・企業規模計" accent="#16a34a" />
              ...
              <KpiCard icon={<Users size={13} color="#7c3aed" />} label="平均年齢" value={fmtFixed(rep.age, 1, '歳')} sub={`勤続 ${fmtFixed(rep.tenure_years, 1, '年')}`} />
                    ...
                    return (
                      <tr key={r.enterprise_size} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFC', borderBottom: i < sizeRows.length - 1 ? '1px solid #F1F5F9' : 'none' }}>
                        <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: isTotal ? 700 : 500, color: '#0F172A', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <Building2 size={13} color={isTotal ? '#16a34a' : '#94A3B8'} />{r.enterprise_size}
                        </td>
                        <td style={{ padding: '11px 14px', fontSize: 14, fontWeight: 700, color: '#16a34a', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{fmtWan(r.annual_income)}</td>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
            {ENTERPRISE_ORDER.map(size => (
              <button key={size} onClick={() => setSizeTab(size)} style={{
                ...
                border: sizeTab === size ? '1.5px solid #16a34a' : '1.5px solid #E2E8F0',
                background: sizeTab === size ? '#dcfce7' : '#fff',
                color: sizeTab === size ? '#16a34a' : '#475569',
                cursor: 'pointer', transition: 'all 0.15s',
              }}>{size}</button>
            ))}
          </div>
        ...
        <section style={{ marginBottom: 36 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8 }}>
            {[
              ...
              { href: '/salary/ranking/age-group', label: '年齢別年収ランキング',   icon: <TrendingUp size={14} color="#7c3aed" /> },
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 8 }}>
          <Link href="/salary/ranking/education" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: '#16a34a', fontSize: 14, textDecoration: 'none', fontWeight: 500 }}>
            <ArrowLeft size={15} />学歴ランキング一覧に戻る
          </Link>
        </div>
